...
- [ ] **A-1.** `http.js`의 `request()` import 추가
- [ ] **A-2.** `fetchAnimals()` 함수 본문을 `request('/api/animals')` 로 교체
...
- [ ] **B-1.** `fetchSido()` 함수 본문을 `request('/api/codes/sido')` 로 교체
...
- [ ] **C-1.** `fetchSigungu(sidoCode)` 함수 본문을 `request('/api/codes/sigungu?uprCd=' + sidoCode)` 로 교체
...
- [ ] **D-1.** `fetchShelters(sigunguCode)` 함수 본문을 `request('/api/codes/shelters?orgCd=' + sigunguCode)` 로 교체
...
- [ ] **E-2.** 백엔드 엔드포인트 생기면 `fetchLostAnimals()` 교체
...
- [ ] **F-1.** `useState`, `useEffect` import 추가
- [ ] **F-2.** `sheltersDummy` import 제거
- [ ] **F-3.** `shelters` state 선언: `const [shelters, setShelters] = useState([])`
- [ ] **F-4.** `useEffect` 안에서 `request('/api/shelters')` 호출 후 결과를 `setShelters()`에 저장
  ```js
  ...
  ```
- [ ] **F-5.** `<KakaoMap shelters={shelters} />` 로 props 연결
  - 백엔드 `ShelterResponse` DTO 필드: `{ id, name, address, tel, lat, lng, weekStartTime, weekEndTime, closedDays, targetAnimals }`
...
- [ ] **G-1.** `fetch('/mock/animals.json')` 직접 호출 부분 제거
- [ ] **G-2.** `api/animals.js` 의 `fetchAnimals()` import
- [ ] **G-3.** `useEffect` 안에서 `fetchAnimals()` 호출로 교체
  ```js
  useEffect(() => {
      fetchAnimals()
...
- [ ] **H-2.** `api/favorites.js` 의 `addFavorite`, `getFavorites` import
- [ ] **H-3.** `LikeCnt(Id)` 함수 내부에 `addFavorite(Id)` API 호출 추가
  - 로그인 필요 API이므로 → 미로그인 시 401 에러 → catch에서 조용히 처리 (또는 로그인 유도 메시지)
  - LocalStorage 업데이트는 API 성공 콜백 안에서 수행 (실패 시 LocalStorage만 변하는 문제 방지)
- [ ] **H-4.** 컴포넌트 마운트 시 `getFavorites()` 호출해서 서버의 찜 목록으로 `likeAnimal` 초기값 설정
...
- [ ] **H-5.** 날짜 기반 LocalStorage 초기화 로직 (`liked_date`) 제거 여부 결정
