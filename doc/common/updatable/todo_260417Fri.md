...
-  **F-04, F-06 동물 상세 및 지도**
  - [ ] 곽선아) 동물 상세 모달/페이지 및 하트 버튼 (API 연동 전 LocalStorage 임시 처리)
  - [ ] 정성오) Kakao Maps SDK 연동 (앱키 발급) 및 더미 보호소 마커 렌더링
- [ ] **F-05 설문 기반 매칭 & F-07 인증 폼**
  - [ ] 정성오) 5문항 설문 UI 및 응답 데이터 -> 필터 파라미터 변환 JS 작성
  - [ ] 김은솔) 회원가입/로그인 폼 UI 및 **순수 JS 정규식 유효성 검사** 로직
...
- [ ] **Should 기능 고도화 (시간 여유 순으로 우선순위 적용)**
  ...
  - [ ] 정성오) 3순위: `S-2` 분실동물 탭
  - [ ] 정성오) 4순위: `S-7` 설문 기록 탭
  - [ ] 정성오) 5순위: `S-5` 커뮤니티 게시판 (POSTS, COMMENTS 테이블 CRUD - 공수 가장 큼)
  - [ ] 정성오) 6순위: `S-3` Gemini AI 추천 1줄 (API 불안정성 고려 후순위)
- [ ] **최종 점검 및 배포 준비**
