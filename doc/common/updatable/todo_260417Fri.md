...
- [ ] **인증 API 연동 (F-07)**
  - [ ] F-1~F-4 회원가입/로그인 등 인증 API 구현 완료
  - [ ] 프론트: Mock 데이터 제거 및 실제 회원가입/로그인 통신 교체 (세션 기반)
- [ ] **[A, C, D] 구조동물 및 드롭다운/지도 API (F-01,02,06)**
  - [ ] A-1 목록 조회 API(JPQL 필터 적용), A-2 단건 상세 API 구현
  - [ ] C-1~C-3 지역/보호소 코드 API (**서버 시작 시 1회 캐싱 적용**)
  - [ ] D-1 전국 보호소 목록+위경도 API 구현
- [ ] **[B, E, G] 관심동물, 설문, 통계 및 동기화 (F-04,05,08)**
  - [ ] B-1~B-3 관심 동물 CRUD API 구현 및 프론트 하트 버튼 실연동
  - [ ] E-1 설문 파라미터 -> 동물 목록 쿼리 변환 레이어 (A-1 재활용)
  - [ ] G-1, G-2 통계 API 및 프론트 랜딩 차트 실연동
  - [ ] 수동 공공 API 동기화 엔드포인트 (`POST /api/admin/sync`) 개발 (페이지네이션 처리 포함)
- [ ] **전역 연동 및 예외 처리**
...
- [ ] **최종 점검 및 배포 준비**
  ...
  - [ ] **교수님 감점 방지용 코드 검수 (AI스러운 코드 수정, 인라인 CSS 제거, 폰트/여백 위계 점검)**